...
$proxy_list = mysql_query("SELECT * FROM tools_proxy WHERE status = 'live' AND conn_time < 3 AND type = 'https' ORDER BY updated desc");
while($row_proxy = mysql_fetch_array($proxy_list)) {
	$proxy_ip .= "<option value=".$row_proxy['proxy'].">".$row_proxy['proxy']." [".$row_proxy['location']."] - ".$row_proxy['type']."</option>";
}
	...
	<form action="includes/process.php?action=edit-browser" method="post">
		...
		<table cellpadding="2" cellspacing="0" align="center" class="large-table">
			...
			<tr>
				...
				<table cellpadding="2" cellspacing="0" align="center" class="large-table">
			<tr>
				<th colspan="2">PROXY IP </th>
			</tr>
			<tr>
				<td>Choose proxy IP:</td>
				<td><select name="proxy_ip">{$proxy_ip}</select></td>
			</tr>
			<tr>
				<td>Your current IP:</td>
				<td><input type="text" disabled="disabled" value="{$_SESSION['custom_browser']['proxy_ip']}"></td>
			</tr>
			<tr>
				<td colspan="2" class="small-note"><b>Note:</b> If proxy is not working, please choose another one.</td>
			</tr>
		</table>
